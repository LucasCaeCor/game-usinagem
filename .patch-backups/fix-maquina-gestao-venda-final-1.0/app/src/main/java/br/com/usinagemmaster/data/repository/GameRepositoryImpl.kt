...
class GameRepositoryImpl @Inject constructor(
    ...
    override fun production(): Flow<ProductionSnapshot> = combine(
        machineDao.observeAll(), employeeDao.observeAll()
    ) { machines, employees -> calculateProduction(machines, employees) }
    ...
    override suspend fun accelerateProduction10Minutes(): Result<Long> = runCatching {
        simulationMutex.withLock {
            ...
            val snapshot = calculateProduction(machineDao.getAll(), employeeDao.getAll())
    ...
    private suspend fun simulateProduction(
        ...
        val snapshot = calculateProduction(machines, employees)
    ...
    ): ProductionSnapshot = ProductionEngine.calculate(
