    ...
    Card(
        ...
        BoxWithConstraints(
            ...
            Canvas(
                ...
                val visualScale = (layout.tileW / 70f).coerceIn(1.12f, 2.8f)
...
private fun DrawScope.studioOperatorAtMachine(
    ...
    val inspectStep = if (operating && cycle in .64f..90f) sin(((cycle-.64f)/.26f) * PI).toFloat() else 0f
    val approach = if (operating) sin(cycle * PI).toFloat().coerceAtLeast(0f) else 0f
    ...
    studioWorker(
        ...
        employee = employee,
        phase = phase * 1.35f,
        walking = operating && (cycle < .18f || cycle > .82f),
        carrying = false,
        scale = scale,
        workLean = workLean,
