...
fun FactoryLiveSceneStudio(
    ...
    val waiting = employees.filter { it.assignedMachineId == null || it.assignedMachineId !in operatingIds }
    ...
    Card(
        ...
        BoxWithConstraints(
            ...
            val heightPx = with(density) { 650.dp.toPx() }
            ...
            Canvas(
                ...
                val visualScale = (layout.tileW / 72f).coerceIn(1.05f, 2.7f)
                ...
                sorted.forEachIndexed { index, machine ->
                    ...
                    studioMachineBay(point, layout, operating, machine.condition)
                    studioMachine(
                        center = point,
                ...
                // O dono da fábrica é o personagem criado pelo jogador. Ele usa o
                // corredor lateral e nunca atravessa a célula de uma máquina.
                val ownerT = studioTriangle((logisticsPhase * .73f + .58f) % 1f)
                val ownerBase = Offset(
            ...
            Surface(
                modifier = Modifier.align(Alignment.BottomEnd).padding(12.dp).clickable { showCopa = !showCopa },
                shape = RoundedCornerShape(12.dp),
                color = if (showCopa) Color(0xFF5D431D) else Color(0xE80A1216),
                border = BorderStroke(1.dp, Color(0xFFFFC766).copy(alpha = .55f))
            ) {
                Row(
                    ...
                    Text("Copa", color = Color(0xFFFFD88D), style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Bold)
                    if (coffeeEmployees.isNotEmpty()) {
                        Text("${coffeeEmployees.size}", color = Color(0xFF1B1406), fontWeight = FontWeight.Black,
                            modifier = Modifier.background(Color(0xFFFFC766), RoundedCornerShape(999.dp)).padding(horizontal = 6.dp, vertical = 1.dp))
                    }
                }
            }
            ...
            if (showCopa) {
                StudioCopaPanel(
                    ...
                    modifier = Modifier.align(Alignment.BottomEnd).padding(end = 12.dp, bottom = 58.dp),
                    phase = workPhase
                )
            }
        }
...
private fun DrawScope.studioMachineBay(center: Offset, layout: StudioLayout, operating: Boolean, condition: Int) {
...
private fun DrawScope.studioOperatorAtMachine(
    ...
    val inspectStep = if (operating && cycle in .66f..88f) sin(((cycle-.66f)/.22f) * PI).toFloat() else 0f
    val body = base + Offset(dir * workLean * 5.5f * scale + dir * inspectStep * 4f * scale, 0f)
    val armTarget = machine + Offset(-dir * 13f * scale, -5f * scale)
    ...
    studioWorker(
        ...
        scale = scale,
        workLean = workLean,
        armTarget = if (operating) armTarget else null
    )

    if (!operating && cycle > .52f) {
...
private fun studioWorkerPalette(employee: EmployeeEntity): StudioWorkerPalette {
    val code = employee.legendaryCode
    return when (code) {
        "tatu_banhado" -> StudioWorkerPalette(Color(0xFF37464C), Color(0xFFE3A63D), Color(0xFFD49B32), Color(0xFFB97A56), 1.05f, 1.00f)
        "kendao" -> StudioWorkerPalette(Color(0xFF243E55), Color(0xFFFFC65C), Color(0xFFF1BD4B), Color(0xFFB87957), 1.10f, 1.03f)
        "chupa_engole" -> StudioWorkerPalette(Color(0xFF4B3433), Color(0xFFF1924A), Color(0xFF353A3C), Color(0xFFC48660), 1.02f, 1.00f)
        "moskitao" -> StudioWorkerPalette(Color(0xFF263E46), Color(0xFF6ED2BA), Color(0xFFF0C44F), Color(0xFF9E684A), .92f, 1.02f)
        "nikao_narizudo" -> StudioWorkerPalette(Color(0xFF25374A), Color(0xFF65B7E8), Color(0xFFFFFFFF), Color(0xFFB77955), .98f, 1.00f)
        "gumersvaldo" -> StudioWorkerPalette(Color(0xFF222B36), Color(0xFF58C6E0), Color(0xFF252B2E), Color(0xFFC38A68), 1.00f, 1.00f)
        "magrao" -> StudioWorkerPalette(Color(0xFF344433), Color(0xFFB9D85D), Color(0xFFE2BA46), Color(0xFFA46C4C), .82f, 1.08f)
        "pedrao" -> StudioWorkerPalette(Color(0xFF463A36), Color(0xFFFFA047), Color(0xFF34383A), Color(0xFFB57752), 1.18f, 1.05f)
        "nelsinho_treme_treme" -> StudioWorkerPalette(Color(0xFF38404E), Color(0xFFE3BE59), Color(0xFFDFB847), Color(0xFFB77A59), .95f, .98f)
        "merciao" -> StudioWorkerPalette(Color(0xFF364956), Color(0xFF7FD5CB), Color(0xFFE8C04B), Color(0xFFC28B67), .98f, 1.00f)
        "bodybuilder" -> StudioWorkerPalette(Color(0xFF283B34), Color(0xFFF0B84A), Color(0xFF303638), Color(0xFFA96D4E), 1.32f, 1.08f)
        else -> StudioWorkerPalette(Color(0xFF31434D), Color(0xFFEAB943), Color(0xFFE6B843), Color(0xFFB77C5A), 1.00f, 1.00f)
    }
}
...
private fun DrawScope.studioWorker(
    ...
    val tremble = if (employee.legendaryCode == "nelsinho_treme_treme") sin(phase * PI * 18).toFloat() * 1.3f * s else 0f
    val walk = if (walking) sin(phase * PI * 2).toFloat() else 0f
    val bob = if (walking) abs(sin(phase * PI * 2).toFloat()) * 1.6f * s else 0f
    val x = base.x + tremble
    val y = base.y - bob

    // Sombra fixa no chão ajuda o personagem a não parecer flutuar.
    ...
    val head = Offset(x + workLean * 2.5f*s, y - 46f*s)

    // Pernas com passada real.
    ...
    drawCircle(Color(0xFF2B201B).copy(alpha = .55f), 1.2f*s, head + Offset(3.2f*s, 4f*s))

    // Característica visual de alguns lendários.
    if (employee.legendaryCode == "nikao_narizudo") {
        drawLine(palette.skin, head + Offset(4f*s, 5f*s), head + Offset(8f*s, 6f*s), 2f*s, StrokeCap.Round)
    }
    ...
    // Nome curto no peito/abaixo do personagem só quando é lendário.
    if (employee.isLegendary) {
