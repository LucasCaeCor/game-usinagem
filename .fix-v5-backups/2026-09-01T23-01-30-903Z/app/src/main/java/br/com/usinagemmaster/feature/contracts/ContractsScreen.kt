                ...
                items(list, key = { it.id }) { contract ->
                    ...
                    Card {
                        Column(Modifier.padding(16.dp)) {
                            ...
                            if (contract.status == ContractStatus.COMPLETED.name) {
                                Spacer(Modifier.height(4.dp))
                                Text(
                                    "✓ Os próximos pagamentos são liquidados automaticamente e de forma protegida.",
                                    color = MaterialTheme.colorScheme.tertiary,
                                    style = MaterialTheme.typography.bodySmall,
                                    fontWeight = FontWeight.SemiBold
                                )
                                Spacer(Modifier.height(8.dp))
                                OutlinedButton(onClick = { vm.recoverReward(contract) }) {
                                    Text("Não recebeu? Recuperar prêmio")
                                }
                                Text(
                                    "Use apenas para contratos antigos que ficaram concluídos sem crédito no caixa.",
